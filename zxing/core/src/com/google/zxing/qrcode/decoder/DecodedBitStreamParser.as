 ...
 */
 ...
 public static function decode(bytes:Array, version:Version, ecLevel:ErrorCorrectionLevel, hints:HashTable):DecoderResult
  {
	 var readBytes:ByteArray; //add by gujj@2012/08/12
          ...
          if (mode.Equals(Mode.HANZI)) {
            ...
            if (mode.Equals(Mode.NUMERIC)) {
              ...
              decodeAlphanumericSegment(bits, result, count, fc1InEffect);
				...
				//>>-------------------- 中文乱码解决 ----------------------gujj@2012/08/12 add
				readBytes = new ByteArray();
				//<<-------------------- 中文乱码解决 ----------------------gujj@2012/08/12 add
              decodeByteSegment(bits, result, count, currentCharacterSetECI, byteSegments, hints, readBytes);
	...
	//>>-------------------- 中文乱码解决 ----------------------gujj@2012/08/12 add
	var ds:DecoderResult = new DecoderResult(bytes,
								result.toString(),
								byteSegments.isEmpty() ? null : byteSegments,
								ecLevel == null ? null : ecLevel.toString());
	ds.setBytes(readBytes);
	//<<-------------------- 中文乱码解决 ----------------------gujj@2012/08/12 add
    return ds; 
  }
  ...
  private static function decodeByteSegment(bits:BitSource,
    ...
    for (var i:int = 0; i < count; i++) {
		//>>-------------------- 中文乱码解决 ----------------------gujj@2012/08/12 add
		read = bits.readBits(8);		
		readBytes[i] = String.fromCharCode(read);
		bytes.writeByte(read);
	  	//<<-------------------- 中文乱码解决 ----------------------gujj@2012/08/12 add
    }
    var encoding:String;
    //if (currentCharacterSetECI == null) {
